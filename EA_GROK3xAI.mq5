...
void OnTick() {
    if(!TimeFilter() || SpreadTooHigh() || !ATRCheck() || (TimeCurrent() - lastTradeTime < 180)) return;

    int confirmations = CheckConfirmations();
    if(confirmations < 3) return;

    double stoploss = (SL_Pips * _Point) + ATRMultiplier * iATR(_Symbol, PERIOD_M5, 14, 0);
    double rr = confirmations >= 7 ? RR_High : RR_Low;
    double takeprofit = stoploss * rr;
    double risk = (Period() == PERIOD_H4 || Period() == PERIOD_D1) ? Risk_High : Risk_Low;
...
int CheckConfirmations() {
    ...
    if(Close[0] > ema20 || Close[0] < ema20) conf++;
